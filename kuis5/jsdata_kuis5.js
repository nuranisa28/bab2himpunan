...
function readlah() {
    var task = firebase.database().ref("kontrolkuis5/");
    let tmp = document.querySelector('body');

    task.on("child_added", function (data) {
        var taskvalue = data.val();

        if ((taskvalue.id == "99387620") && (taskvalue.nilai == 0)) {
...
dat.open('GET', 'data_kuis5.json', true);
dat.send();
...
function createTask(sekolah, nama, kelas, nilai, waktunya, hari, jwb) {
    ...
    let db = firebase.database().ref("kuis5_sebelumtes/" + counter);
    db.set(task);
