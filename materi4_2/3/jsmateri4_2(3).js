...
function cek31(){
  ...
  var anggota = ['merah', 'kuning', 'putih', 'ungu', 'biru', 'hijau', 'jingga'];
  var himpunan = ['P','K'];
  var ada1 = 0;
  var ada2 = 0;
  var benar = 0;
  ...
  if(himp1.value.length>0 || himp2.value.length>0 || ang.value.length>0){
    ...
    if(himp1.value != "P" && himp1.value != "K"){
      himp1.className += " boxmerah";
    }
    ...
    if(benar==2){
      ...
      document.getElementById('ket31').innerHTML = "<br><span class='bnr'>Jawaban kamu benar.</span> " + himp1.value + " &cup; " + himp2.value + " = {" + ang2 +"}.";
    } else{
